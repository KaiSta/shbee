package examples.airlinetickets;

import java.io.FileOutputStream;
import java.io.FileNotFoundException;

/**
 * Created by IntelliJ IDEA. User: amit rotstein I.D: 037698867 Date: Oct 17,
 * 2003 Time: 1:02:13 PM To change this template use Options | File Templates.
 */
public class Airlinetickets implements Runnable {
	...
	public Airlinetickets(String fileName, String Concurency) {
		...
		if (Concurency.equals("average"))
			Num_of_tickets_issued = 100;
		if (Concurency.equals("lot"))
			Num_of_tickets_issued = 5000;
		go(Concurency);
	}

	public void go(String Concurency) {
		Maximum_Capacity = Num_of_tickets_issued - (Num_of_tickets_issued) / 10; // issuing
																					// 10%
																					// more
																					// tickets
																					// for
		...
		System.out.println("The airline issued " + Num_of_tickets_issued + " tickets for " + Maximum_Capacity
				+ " seats to be sold.");
		/**
		 * starting the selling of the tickets: "StopSales" indicates to the
		 * airline that the max capacity was sold & that they should stop
		 * issuing tickets
		 */
		...
		for (int i = 0; i < Num_of_tickets_issued; i++) {
			...
			/**
			 * first the airline is checking to see if it's agents had sold all
			 * the seats:
			 */
			if (StopSales) {
				Num_Of_Seats_Sold--;
			...
			/**
			 * THE BUG : StopSales is updated by the selling posts ( public void
			 * run() ), and by the time it is updated more tickets then are
			 * alowed to be are sold by other threads that are still running
			 */
