...
import examples.boundedbuffer.Buffer;
import java.io.*;
...
//            System.out.println("----------------------");
     ...
     */
    public boolean WaitForBug(int numOfThreads){
...
/** The Producer thread  */
 class Producer implements Runnable{
    ...
    public void run(){
        try {
            for (int i=0;i<BoundedBuffer._ITEMS_PRODUCED;i++){
                buffer.enq(name+"["+String.valueOf(i)+"]");
            }
        }catch (InterruptedException ex) {
        }finally{
            synchronized(this) { BoundedBuffer._finish++;} // _finish is a common resource
        }
    }
}
...
/** The Consumer thread  */
 class Consumer implements Runnable{
    ...
    public void run(){
        try {
            for (int i=0;i<BoundedBuffer._ITEMS_PRODUCED*2;i++){
                buffer.deq(name);
            }
        }catch (InterruptedException ex) {
